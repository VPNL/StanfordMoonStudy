...
% because this is an image the rows are y axis and columns are x axis 
yTickVec=round(log2(maxAngle)):-1:round(log2(minAngle));

log2_angleRange=linspace(round(log2(minAngle)),round(log2(maxAngle)),length(yTickVec)*20); 
...
log2_distanceRange=linspace(round(log2(minDistance)),round(log2(maxDistance)),length(xTickVec)*20);
for d=1:length(log2_distanceRange)
    for a=1:length(log2_angleRange)
        log2PM(a,d)=intercept + beta_log_real_angle * log2_angleRange(a) + beta_log_distance * log2_distanceRange(d);
    end
end
...
imagesc(flipud(2.^log2PM), [.75 4]);colormap(turbo)
cb=colorbar; ylabel(cb,'Perceptual Magnification','FontSize',14) 

xlabel ('Distance (m) log scale')
ylabel ('Visual Angle (deg) log scale')
spaceYticks=length(log2_angleRange)/[length(yTickVec)];
spaceXticks=length(log2_distanceRange)/[length(xTickVec)];
set(gca,'YTick',1:spaceYticks:length(log2_angleRange),'YTickLabel',2.^yTickVec);
set(gca,'XTick',1:spaceXticks:length(log2_distanceRange),'XTickLabel',2.^xTickVec);
axis('equal')
...
% 
figh=figure('Color',[1 1 1],'Units','normalized','Position',[0 0 1 .35],'Name',[basename '_' task])
