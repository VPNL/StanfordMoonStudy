...
for i=1:nObjects
 ...
 % save figure
clear filenamePNG
filenamePNG=fullfile('.','Results','DataChecks', [basename,'_ratio_by_id_', num2str(nsubjects),'.png']);
...
filenameTXT=fullfile('.','Results','DataChecks','QuadLongMeanData.txt')
...
for t=1:nTasks
    ...
    for i=1:nObjects
        ...
        idx=find(contains(task_data.Measurement_Type,uniqueObject(i) ));
        if ~isempty(idx)
           counter=counter+1;
...
filenamePNG=fullfile('.','Results','DataChecks', [basename,'_boxplots_by_task_', num2str(nsubjects),'.png']);
exportgraphics(figh,filenamePNG,'Resolution',600);
fclose(fileID)
