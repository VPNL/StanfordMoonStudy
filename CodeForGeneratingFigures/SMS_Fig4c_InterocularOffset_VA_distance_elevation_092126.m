% Paper2_Quad_InterocularOffset_VA_distance_elevation_081726
...
switch colorMode
    case "stereoscore"
        colorResultsFolder = ['InterOcularOffset_' quadBasename '_StereoScore'];
        colorbarLabel = 'Normed stereo score';
    case "clinicalnotes"
        colorResultsFolder = ['InterOcularOffset_' quadBasename '_ClinicalNotes'];
        colorbarLabel = 'Clinical notes';
    case "id"
        colorResultsFolder = ['InterOcularOffset_' quadBasename '_ID'];
...
resultsDir = fullfile(expDir, 'Figures','Fig4c' , colorResultsFolder);
...
%% test relation between offset and distance and elevation
...
%% test relation between offset visual angle and distance and elevation
SupFigDir=fullfile(resultsDir,'SupFig_VA_D_E');
if ~exist(SupFigDir, 'dir')
    mkdir(SupFigDir);
end

[distanceRI, elevationRI, distanceElevationRI, ...
    angleDistanceElevationRI, angleElevationRI, angleDistanceRI] = ...
    Quad_PerceivedOffset_by_VADistanceElevation( ...
    modelTbl, tableName, SupFigDir, saveLME, cmap, ...
    sortedIdx, transformId, 1, uniqueID, ...
    removeOutlierParticipants, secondYAxisColor, colorConfig);

   [distanceSingleRI,     distanceRS, ...
    elevationSingleRI, elevationRS, ...
    angleSingleRI, angleRS, ...
    distanceStereoRI, elevationStereoRI, angleStereoRI] = ...
...
summaryFile = fullfile(SupFigDir ,[tableName '_lme_summary.csv']);
summaryStore = ...
    write_lme_fixed_effects_summary_csv(quadBasename, ...
    {'logOffset_by_logDistance', ...
    'logOffset_by_logElevation', ...
    'logOffset_by_logDistanceNElevation', ...
    'logOffset_by_logAngleDistanceElevation', ...
    'logOffset_by_logAngleElevation', ...
    'logOffset_by_logAngleDistance'}, ...
    {distanceRI, elevationRI, distanceElevationRI, ...
    angleDistanceElevationRI, ...
    angleElevationRI, angleDistanceRI}, summaryFile);
modelStore = struct( ...
    'DistanceRI', distanceRI, ...
    'ElevationRI', elevationRI, ...
    'DistanceElevationRI', distanceElevationRI, ...
    'AngleDistanceElevationRI', angleDistanceElevationRI, ...
    'AngleElevationRI', angleElevationRI, ...
    'AngleDistanceRI', angleDistanceRI, ...
    'DistanceSingleRI', distanceSingleRI, ...
    'ElevationSingleRI', elevationSingleRI, ...
    'DistanceRS', distanceRS, ...
    'ElevationRS', elevationRS, ...
    'DistanceStereoRI', distanceStereoRI, ...
    'ElevationStereoRI', elevationStereoRI, ...
    'AngleSingleRI', angleSingleRI, ...
    'AngleRS', angleRS);
