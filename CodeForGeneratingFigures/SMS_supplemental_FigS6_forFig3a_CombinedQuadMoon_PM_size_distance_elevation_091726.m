...
% SMS_supplemental_Fig3a_SupplementalFigure_CombinedQuadMoon_PM_size_distance_elevation_091526
close all; clear all;
...
dataDir='/Users/kalanit/Projects/StanfordMoonStudy/Data/';

combinedDir=fullfile(dataDir,'Combined');
if ~exist('combinedDir','dir')
   mkdir(combinedDir)
end

ResultsDir=fullfile(expDir,'Figures','Supplemental_FigS6_for3a_S_D_E');
...
MoonExpDir=fullfile(dataDir, 'MoonStudy');
...
colNames=   {'ID', 'Real_Visual_Angle','Distance','Elevation' ,...
            'Task','Reported_Visual_Angle','Ratio_Visual_Angle','Size'}
% transform moon distances from km to meters
all_moon_data.Distance=1000*all_moon_data.Distance;
h=height(all_moon_data);
all_moon_data.Size=3476*1000*ones(h,1); % Moon Diameter in m;
moon_data_perceptual=table(all_moon_data.ID, all_moon_data.Real_Visual_Angle,all_moon_data.Distance,all_moon_data.Elevation,...
                       all_moon_data.Task, all_moon_data.Reported_Visual_Angle, all_moon_data.Ratio_Visual_Angle,all_moon_data.Size,...
...
% quad study
% set dirs
QuadExpDir=fullfile(dataDir, 'QuadStudy');
QuadFile='Perceptual_StanfordQuadStudyDataProcessed0420.csv';
QuadBasename = [erase(QuadFile,'.csv')] ;
all_quad_data = add_diameter_height(fullfile(QuadExpDir,QuadFile), fullfile(QuadExpDir,[QuadBasename '_wSize.csv' ])); 
...
if ObserverFlag
    ...
    all_quad_data.Size=all_quad_data.Width/100;% transform distances from cm to m
...
quad_data_perceptual=table(all_quad_data.ID,all_quad_data.Real_Visual_Angle,all_quad_data.Distance, all_quad_data.Elevation,...
                       all_quad_data.Task, all_quad_data.Reported_Visual_Angle,all_quad_data.Ratio_Visual_Angle,all_quad_data.Size,...
                       'VariableNames',colNames);
...
combined_basename=['combined_withS_' MoonBasename '_' QuadBasename];
combinedfileName=fullfile(combinedDir,[combined_basename '.csv']);

writetable(combined_data,combinedfileName);
plot_S_D_E_parameters(combined_data,combined_basename,ResultsDir);
...
%             Quad_PM_by_task_S_D_E(combined_data, combined_basename, ResultsDir, saveLME, cmap_combined, sorted_idx, elevationTransform);

% triple factor model plot fit by factor
lme_full = fit_PM_full_S_D_E_model(combined_data, [combined_basename '_S_D_E_model'], ResultsDir, saveLME, cmap_combined, sorted_idx, elevationTransform);

% outCsvFile = fullfile(ResultsDir, [combined_basename '_S_D_E_summary.csv']);
% Quad_export_S_D_E_summary_csv('combined_quad_moon', lower(task), ...
%         lme_logPM_by_logSize, lme_logPM_by_logDistance, lme_logPM_by_logElevation, ...
%         lme_logPM_by_logSizeNDistance, lme_logPM_by_logSizeNElevation, lme_logPM_by_logDistanceNElevation, ...
