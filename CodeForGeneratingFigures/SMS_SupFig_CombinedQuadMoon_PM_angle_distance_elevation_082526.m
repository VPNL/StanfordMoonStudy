...
% SMS_SupFig_CombinedQuadMoon_PM_angle_distance_elevation_082526
close all; clear all;
SMS_setCodePath;
% set dirs
...
ResultsDir=fullfile(expDir,'Figures','SupFig_CombinedAdjusted');
...
%  the moon data is in km and the quad data is in cm 

MoonExpDir='/Users/kalanit/Projects/StanfordMoonStudy/Data/MoonStudy/';
MoonFile='FullMoonDataLong090225.csv'; % includes both percpetual and adjusted tasks
...
moon_data=table(all_moon_data.ID, all_moon_data.Real_Visual_Angle,all_moon_data.Distance,all_moon_data.Elevation,...
                       all_moon_data.Task, all_moon_data.Reported_Visual_Angle, all_moon_data.Ratio_Visual_Angle,...
                      'VariableNames',colNames); 
hh=height(moon_data);
for h=1:hh
    moon_data.Study(h)={'MoonStudy'};
end
...
QuadFile='MatchingProcessedData0421.csv'; % includes both perceptual and adjusted tasks
...
quad_data=table(all_quad_data.ID,all_quad_data.Real_Visual_Angle,all_quad_data.Distance, all_quad_data.Elevation,...
                       all_quad_data.Task, all_quad_data.Reported_Visual_Angle,all_quad_data.Ratio_Visual_Angle,...
                       'VariableNames',colNames);
hh=height(quad_data);
for h=1:hh
    quad_data.Study(h)={'QuadStudy'};
end

%% organize combined data across moon and quad
combined_data=[moon_data;quad_data];
...
%% separate combined data by perceptual and adjusted tasks
task='Adjusted';
task_i=find(strcmp(combined_data.Task,task));
combined_data_adjusted=combined_data(task_i,:);

task='Perceptual';
task_i=find(strcmp(combined_data.Task,task));
combined_data_perceptual=combined_data(task_i,:);

% sort subjects by PM in perceptual task
for i=1:nsubjects
    idx=find(combined_data_perceptual.ID==uniqueID(i));
    meanPM(i)=mean(combined_data_perceptual.Ratio_Visual_Angle(idx));
end
...
taskData = struct('Adjusted', combined_data);
...
% test all models: single, dual, and triple factor models; 
% plot single factor fits for the combined data and adjusted task
[lme_logPM_by_logAngle,lme_logPM_by_logDistance,lme_logPM_by_logElevation,...
 ...
 lme_logPM_by_logAngleNDistanceNElevation] = ...
            Quad_PM_by_task(combined_data_adjusted, combined_basename, ResultsDir, saveLME, cmap_combined, sorted_idx, elevationTransform);

% triple factor model plot fit by factor for combined data and adjusted
lme_full = fit_PM_full_VA_D_E_model(combined_data_adjusted, [combined_basename '_VA_D_E_model'], ResultsDir, saveLME, cmap_combined, sorted_idx, elevationTransform);
...
OutName = [ 'SupFig_PM_full_VA_D_E_modelbytask_' combined_basename];
[lme, interactionStats, modelComparison] = fit_PM_full_VA_D_E_modelbytask( ...
    combined_data,  OutName, ResultsDir, 1,cmap_combined, sorted_idx,  elevationTransform);
