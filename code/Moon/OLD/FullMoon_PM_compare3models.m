function results = FullMoon_PM_compare3models(dataDir,datafile,ResultsDir,task,recomputeSort,saveLME)
...
%   3) Ratio_Visual_Angle ~ Elevation + (Elevation|ID)
%
...
comparisonTable = table( ...
    ...
    [p1; p2; p3], ...
    string({sigstars(p1); sigstars(p2); sigstars(p3)}), ...
    [rmse1; rmse2; rmse3], ...
    [r2_1; r2_2; r2_3], ...
    repmat(nsubjects,3,1), ...
    repmat(nobs,3,1), ...
    'VariableNames', {'Formula','Response','AIC','BIC','LogLikelihood','pValue','Sig','RMSE_rawRatio','R2_rawRatio','nSubjects','nObs'});
...
title(ax1, {displayFormula1, sprintf('p=%s, ns=%d', formatP(p1), nsubjects)}, 'Interpreter','none')
...
title(ax2, {displayFormula2, sprintf('p=%s, ns=%d', formatP(p2), nsubjects)})
...
title(ax3, {displayFormula3, sprintf('p=%s, ns=%d',  formatP(p3), nsubjects)})
...
set(axh,'FontSize',12,'FontName','Avenir')
xlim(axh,[xMin xMax])
ylim(axh,[yMin yMax])
