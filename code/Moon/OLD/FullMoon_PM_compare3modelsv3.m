function results = FullMoon_PM_compare3modelsv3(dataDir,datafile,ResultsDir,task,recomputeSort,saveLME)
...
all_data.logElevationRAD = log2(1 + pi*all_data.Elevation/180);
...
% for radians
xMinRAD = 0;
xMaxRAD=max(all_data.ElevationRAD);
markerScale = 36;

% ----------------------
% fit the 3 requested models
% ----------------------
fitFormula1 = 'logRatio ~ logElevation + (1|ID)';
fitFormula2 = 'logRatio ~ logElevationRAD + (1|ID)';
fitFormula3 = 'Ratio_Visual_Angle ~ ElevationRAD + (1|ID)';

displayFormula1 = 'logRatio ~ log2(1+Elevation) + (1|ID)';
displayFormula2 = 'logRatio ~ log2(1 + Elevation_{RAD}) + (1|ID)';
displayFormula3 = 'Ratio_Visual_Angle ~ Elevation_{RAD} + (1|ID)';
...
titleFormula1 = localPowerFormula(2^fe1(1), '1 + Elevation', fe1(2));
titleFormula2 = localPowerFormula(2^fe2(1), '1 + Elevation_{RAD}', fe2(2));
...
[yFit1, yLow1, yHigh1, p1] = localPredictBand(model1, xFit, 'logelevation');
% need to transform x to radian for second and third model
xFitRAD = linspace(xMinRAD,xMaxRAD,300)';
[yFit2, yLow2, yHigh2, p2] = localPredictBand(model2, xFitRAD, 'logelevation');
[yFit3, yLow3, yHigh3, p3] = localPredictBand(model3, xFitRAD, 'linear');

% predictions at observed elevations for comparison metrics on a common scale

[predObs1, ~, ~, ~] = localPredictBand(model1, all_data.Elevation, 'logelevation');
[predObs2, ~, ~, ~] = localPredictBand(model2, all_data.ElevationRAD, 'logelevation');
[predObs3, ~, ~, ~] = localPredictBand(model3, all_data.ElevationRAD, 'linear');
...
scatter(ax2, all_data.ElevationRAD, obsRatio, markerScale, subjectcolor, 'o', 'filled');
if p2<0.05 % plot line fit only if significant
    fill(ax2, [xFitRAD; flipud(xFitRAD)], [yLow2; flipud(yHigh2)], 1, ...
    'facecolor','k', 'edgecolor','none', 'facealpha',0.12);
    plot(ax2, xFitRAD, yFit2, 'k-', 'LineWidth', 3);
end
plot(ax2, [xMinRAD xMaxRAD], [1 1], 'Color',[.8 .8 .8], 'LineWidth',2);
localFormatAxisRAD(ax2, xMinRAD, xMaxRAD, yMin, yMax)
title(ax2, {titleFormula2, sprintf('p=%s, ns=%d', formatP(p2), nsubjects)})

ax3 = subplot(1,3,3); hold(ax3,'on')
scatter(ax3, all_data.ElevationRAD, obsRatio, markerScale, subjectcolor, 'o', 'filled');
% plot line fit only if significant
if p3<0.05
    fill(ax3, [xFitRAD; flipud(xFitRAD)], [yLow3; flipud(yHigh3)], 1, ...
    'facecolor','k', 'edgecolor','none', 'facealpha',0.12);
    plot(ax3, xFitRAD, yFit3, 'k-', 'LineWidth', 3);
end
plot(ax3, [xMinRAD xMaxRAD], [1 1], 'Color',[.8 .8 .8], 'LineWidth',2);
localFormatAxisRAD(ax3, xMinRAD, xMaxRAD, yMin, yMax)
title(ax3, {titleFormula3, sprintf('p=%s, ns=%d', formatP(p3), nsubjects)})
...
function localFormatAxisRAD(axh, xMin, xMax, yMin, yMax)
set(axh,'FontSize',16,'FontName','Avenir')
xlim(axh,[xMin xMax])
ylim(axh,[yMin yMax])
xlabel(axh,'Moon Elevation (radians)')
ylabel(axh,'Perceptual Magnification')
box(axh,'off')
end
% =======================================================================
...
if slopeCoeff >= 0
    s = sprintf('PM = %s + %s*Elevation_{RAD}', formatCoef(interceptCoeff), formatCoef(slopeCoeff));
else
    s = sprintf('PM = %s - %s*Elevation_{RAD}', formatCoef(interceptCoeff), formatCoef(abs(slopeCoeff)));
end
end
