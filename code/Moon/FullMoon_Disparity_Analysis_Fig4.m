...
if model_comp.pValue <0.05
    fprintf('Random slopes and Random intercepts model is better, pval=%f\n',model_comp.pValue(2))
else
...
%% now let's repeat with relative disparity
IDD=disparity_data.ID;
uniqueIDD=unique(IDD);
nsubjectsD=length(uniqueIDD);
fprintf('%d subjects for full moon disparity data\n ',nsubjectsD);
...
for i = 1:nsubjectsD
   ...
   subjectRows = find(disparity_data.ID == uniqueIDD(i));
   meanD=mean(disparity_data.Disparity_VA(subjectRows));
   disparity_data.RelativeDisparity(subjectRows)=disparity_data.Disparity_VA(subjectRows)-meanD; 
end

lme_relativedisparity_by_Elevation = fitlme(disparity_data,'RelativeDisparity ~ Elevation  + (1|ID)')
lme_relativedisparity_by_Elevation_RS = fitlme(disparity_data,'RelativeDisparity ~ Elevation  + (Elevation|ID)')
relativedisparity_model_comp=compare(lme_relativedisparity_by_Elevation,lme_relativedisparity_by_Elevation_RS); % loglikelihood test comparing models

disp('Relative disparity by Elevation model comparison')
if relativedisparity_model_comp.pValue <0.05
    fprintf('Random slopes and Random intercepts model is better, pval=%f\n',relativedisparity_model_comp.pValue(2))
else
    fprintf('Random intercepts model is better, pval=%.2f\n',relativedisparity_model_comp.pValue(2))
end
...
if saveLME % save stats table
     savelmefile=fullfile('.',ResultsDir, [basename  '_lme_relativedisparity_by_Elevation.txt']);
     diary(savelmefile)
     lme_relativedisparity_by_Elevation
     lme_relativedisparity_by_Elevation_RS
     relativedisparity_model_comp
...
% Estimate relationship between disparity and Elevation
figRD=figure('Color',[1 1 1],'Units','normalized','Position',[0 0 1 .6],'Name',[basename 'Relative Disparity'])



[reRDEfx,reRDNames,reRDStats] = randomEffects(lme_relativedisparity_by_Elevation_RS);
[feRDEfx,feRDNames,feRDstats] = fixedEffects(lme_relativedisparity_by_Elevation_RS);


RDintercept_indices = contains(reRDNames.Name, '(Intercept)');
RDslope_indices = contains(reRDNames.Name, 'Elevation');
RDrandom_intercepts = reRDEfx(intercept_indices);
RDrandom_slopes=reRDEfx(slope_indices);
RDsubjIntercepts = RDrandom_intercepts + feRDEfx(1);  % subject intercept = fixed + random
RDsubjSlopes = RDrandom_slopes + feRDEfx(2); 

[RDsorted_random_intercepts, RDsortedIdx]=sort(RDrandom_intercepts); % sort participants by intercepts
RDsorted_random_slopes=random_slopes(RDsortedIdx);

% set colormap & markerscale
markerScale=36;
cmapRD=jet(nsubjectsD);
clear RDsubjectcolor;

RDsorteduniqueIDD=uniqueIDD(RDsortedIdx);
for c=1:length(IDD) 
    idx=IDD(c);
    ii=find(idx==RDsorteduniqueIDD);
    RDsorted_idx(c)=ii;
    RDsubjectcolor(c,:)=cmapRD(ii,:);
end
savefile=fullfile('.', ResultsDir, [basename  '_relativedisparity_sortedidx']);
save(savefile ,'RDsorted_idx');

% fixed effects lines
RDinterceptDis=feRDEfx(1);
RDslopeDis=feRDEfx(2);
RDpvalDis=feRDstats.pValue(2);
RDlower_slopeDis=feRDstats.Lower(2);
RDupper_slopeDis=feRDstats.Upper(2);
RDlower_interceptDis=feRDstats.Lower(1);
RDupper_interceptDis=feRDstats.Upper(1);

xvectoru=linspace(0,maxElevation);
xvectord = sort(xvectoru, 'descend');
RDyvectoru=RDslopeDis*xvectoru+RDinterceptDis;
yvector1=RDlower_slopeDis*xvectoru+RDlower_interceptDis;
yvector2=RDupper_slopeDis*xvectord+RDupper_interceptDis;
xvector=[xvectoru xvectord];
yvector=[yvector1 yvector2];
...
% % plot individual subject lines
    for s=1:nsubjectsD
        sortedID=RDsortedIdx(s);
        sindex=find(uniqueIDD==IDD(RDsortedIdx));
        jj=find(disparity_data.ID==uniqueIDD(sindex));
        if length(jj>1)
            sdata=disparity_data(jj,:);
            lower=find(strcmp(sdata.Session,'Lower'));
...
scatter(disparity_data.Elevation, disparity_data. RelativeDisparity ,markerScale, RDsubjectcolor, 'o','filled');

xlim([0 max(disparity_data.Elevation)]);
ylim ([min(disparity_data.RelativeDisparity) max(disparity_data.RelativeDisparity )]);
plot([0 max(disparity_data.Elevation)], [ 0 0] ,'k-')

ylabel ('Relative Disparity (degrees)')
xlabel ('Elevation (degrees)') 
set(gca,'FontSize',20,'FontName','Avenir')

if RDpvalDis<0.001 % nice writing of p-values
    titlestr=sprintf('relative disparity=%.2f+%.2f*elevation,\n p=%5.2e n=%d', RDinterceptDis,RDslopeDis,RDpvalDis, length(unique(IDD)));
elseif RDpvalDis<0.05
    titlestr=sprintf('relative disparity=%.2f+%.2f*elevation, \n p=%.3f n=%d', RDinterceptDis,RDslopeDis,RDpvalDis, length(unique(IDD)));
else
   titlestr=sprintf('relation not significant, p=%.3f\n n=%d',RDpvalDis, length(unique(IDD)));
...
task='Perceptual';
 subplotNum=2
[feEfxRP,feNamesRP,festatsRP,reEfxRP,reNamesRP,reStatsRP] = FullMoon_ratioVrelativedisparity(dataDir, datafile,task, ResultsDir, saveLME, subplotNum, RDsorteduniqueIDD);
%%
task='Adjusted';
 subplotNum=3
[feEfxRA,feNamesRA,festatsR,reEfxRA,reNamesRA,reStatsRA] = FullMoon_ratioVrelativedisparity(dataDir, datafile,task, ResultsDir, saveLME, subplotNum, RDsorteduniqueIDD);

%%
filenamePNG=fullfile(dataDir,ResultsDir,[basename,'_relativedisparity_', num2str(nsubjectsD),'.png']);
