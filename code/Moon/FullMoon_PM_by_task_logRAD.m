...
function [lme_by_logRatio_by_Elevation]=FullMoon_PM_by_task_logRAD(dataDir,datafile,ResultsDir,task, recomputeSort,saveLME)
%
...
all_data.Elevation=pi*all_data.Elevation/180;
...
all_data.ElevationRAD=all_data.Elevation;
all_data.logRatio=log2(all_data.Ratio_Visual_Angle);
all_data.logElevation=log2(all_data.ElevationRAD+1); % add regularization term to elevation as log(0) is not defined and elevation can be zero
%% set colormap
cmap=jet(nsubjects);
markerScale=36;
%%
...
plottedLogModel = lme_by_logRatio_by_Elevation_RS;
if saveLME
   ...
   write_moon_lme_report(savelmefile, ...
       sprintf('%s Moon log PM by log elevation (radians)', task), ...
       summaryLines, ...
...
% linear model
[reEfx_log,reNames_log,reStats_log] = randomEffects(plottedLogModel);
[feEfx_log,feNames_log,festats_log] =fixedEffects(plottedLogModel);
...
plot([0 max(all_data.logElevation)], [0 0],'Color',[.8 .8 .8],'LineWidth',3)
xlim([0 max(all_data.logElevation)])
...
xticks=get(gca,'Xtick');set(gca,'XTickLabel', round(2.^xticks-1,1));
yticks=get(gca,'Ytick');set(gca,'YTickLabel', round(2.^yticks,1));

% estimate function PM= 2^interceptL*elevation^slopeL
interceptL=feEfx_log(1);
slopeL=feEfx_log(2);
xvector=linspace(0,max(all_data.Elevation))%linspace(min(all_data.Elevation),max(all_data.Elevation));  %estimate from 1 degree and up
xvector=linspace(min(all_data.Elevation),max(all_data.Elevation))
yvector=2^interceptL*((xvector+1).^slopeL);
...
% confidence interval
yvector1=2^interceptL*((xvector+1).^lower_slopeL);
xvectord = sort(xvector, 'descend');
yvector2=2^interceptL*((xvectord+1).^upper_slopeL);
xvectorCI=[xvector xvectord];
yvectorCI=[yvector1 yvector2];
fill(xvectorCI, yvectorCI, 1,'facecolor', 'k', 'edgecolor', 'none', 'facealpha', 0.1); % draw confidennce interval
  
plot([0 maxElevation], [1 1],'Color',[.8 .8 .8 ],'LineWidth',3)
xlabel ('Moon Elevation (radians)')
ylabel ('Perceptual Magnification') 
ylim([0 maxRatio])
xlim([0 maxElevation*1.05])
...
lme_ratio_by_ElevationRAD = fitlme(all_data,'Ratio_Visual_Angle~ElevationRAD + (1|ID)')
% subject is a random effect, on intercept & slope
lme_ratio_by_ElevationRAD_RS = fitlme(all_data,'Ratio_Visual_Angle~ElevationRAD + (ElevationRAD|ID)')
% compare models
model_comp=compare(lme_ratio_by_ElevationRAD,lme_ratio_by_ElevationRAD_RS); % model with random slopes is more significant

[reEfx,reNames,reStats] = randomEffects(lme_ratio_by_ElevationRAD_RS);
[feEfx,feNames,festats] =fixedEffects(lme_ratio_by_ElevationRAD_RS);
...
if saveLME
   ...
   write_moon_lme_report(savelmefile, ...
       sprintf('%s Moon perceptual magnification by elevation (radians)', task), ...
       summaryLines, ...
       {lme_ratio_by_ElevationRAD, lme_ratio_by_ElevationRAD_RS}, ...
       {'lme_ratio_by_ElevationRAD', 'lme_ratio_by_ElevationRAD_RS'}, ...
       {model_comp}, ...
       {'model_comp'});
end
...
lower_slopeL=festats_log.Lower(2);
upper_slopeL=festats_log.Upper(2);
xvector=linspace(0,max(all_data.Elevation)); % estimate from 1 degree and up
yvector=2^interceptL*((xvector+1).^slopeL);
...
ylim([0 maxRatio])
xlim([0 maxElevation*1.05])
% i will put the x ticks in degrees rather than radians for
% interpretability
set(gca,'FontSize',28,'FontName','Avenir')
set(gca,'YTick', [0:1:maxRatio],'YtickLabel', [0:1:maxRatio])
Xtick=get(gca,'XTick');
set(gca,'XtickLabel',round(180*Xtick/pi));
%xlabel ('Moon Elevation (radians)')
xlabel ('Moon Elevation (degrees)')
ylabel ('Perceptual Magnification') 

if pvalL < 0.001
        titlestr=sprintf('%s Matching\n PM=%.1f*(1+Elevation_{RAD})^{%.2f}  \n n=%d',task, round(2^interceptL,1),slopeL, nsubjects);
    else
        titlestr=sprintf('%s Matching\n PM=%.1f*(1+Elevation_{RAD})^{%.2f} \n n=%d',task, round(2^interceptL,1),slopeL, nsubjects);
end 
title(titlestr, 'FontSize',18,'FontName','Avenir')

lme_by_logRatio_by_Elevation = plottedLogModel;
...
% so we will estimate the perceived visual angle at an elevation of 0.25 degrees or 0.0044 radians(half the size of the moon) to
% estimate perceived visual angle at horizon

elevation=0.0044; % moon just passed the horizon 
...
elevation=0.0436; % lowest elevation we measured 2.5 degrees or 0.0426 radians
...
elevation=.6981; % highest elevation we measured 40 degrees of 0.6981 radians
