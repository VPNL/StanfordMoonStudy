...
function [lme_by_logRatio_by_Elevation]=FullMoon_PM_by_task_log(dataDir,datafile,ResultsDir,task, recomputeSort,saveLME)
%
...
if saveLME
   ...
   write_moon_lme_report(savelmefile, ...
       sprintf('%s Moon log PM by log elevation', task), ...
       summaryLines, ...
...
%
figh=figure('Color',[1 1 1],'Units','normalized','Position',[0 0 1 .8],'Name',[basename 'Perceptual Magnification vs Elevation'])
...
if saveLME
   ...
   write_moon_lme_report(savelmefile, ...
       sprintf('%s Moon perceptual magnification by elevation', task), ...
       summaryLines, ...
