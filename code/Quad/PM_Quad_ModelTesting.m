...
ResultsDir='/Users/kalanit/Projects/PerceptualMagnification/Paper/ExperimentalData/QuadExperiments/Results/Modeltesting_011526'
...
% % % test without shortdistances
% allDistances=unique(all_quad_data.Distance);
% threshold=20*100;
% jj=find(all_quad_data.Distance>threshold);
% all_quad_data=all_quad_data(jj,:);
% allmeasurements=sort(unique(all_quad_data.Measurement))
% QuadBasename=[QuadBasename '_20nUp'];

%allmeasurements=sort(unique(all_quad_data.Measurement));
% 
% jj=find(~strcmp(all_quad_data.Measurement,'BlueBall_Adjusted_VA'));
% all_quad_data_noBlueBall=all_quad_data(jj,:);
% jj= find( ~strcmp(all_quad_data_noBlueBall.Measurement,'BlueBall_Perceptual_VA') );
% all_quad_data_noBlueBall=all_quad_data_noBlueBall(jj,:);
% allmeasurements=sort(unique(all_quad_data_noBlueBall.Measurement));
% QuadBasename=[QuadBasename '_noBlueBall'];
% all_quad_data=all_quad_data_noBlueBall;
% allmeasurements=sort(unique(all_quad_data.Measurement))
%%
