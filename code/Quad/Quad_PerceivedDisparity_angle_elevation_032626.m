% Quad_PerceivedDisparity_angle_elevation_032626
...
QuadFileName='QuadProcessedDisparityData032626_11pm.csv';
...
ResultsDir=fullfile(expDir,['PerceivedDisparityAngleElevation_' QuadBaseName '_allTransforms']);
ElevationTransforms=[2 5 6];
saveLME=1;
recomputeColorIndex=1;
removeOutlierParticipants=1;
removeZScoreOutliers=0;
zScoreThreshold=3;
version3DiskOnly=1;
...
summaryStore = struct();
lmeStore = struct();
versionLabels = ["AllVersions" "Version3"]; % "Version1" "Version2"
...
for transformId = ElevationTransforms
    ...
    for iVersion = 1:numel(analysisSpecs)
        ...
        summaryTbl = Quad_PercievedDisparity_by_angleElevation_LME_summary(QuadBasename, ...
            lme_logPM_by_logAngle, lme_logPM_by_logElevation, lme_logPM_by_logAngleNElevation, outCsvFile);

        lme_full = fit_PerceivedDisparity_by_angleElevation_lme(tblSubset, [tblName '_fullmodel_fit'], VersionResultsDir, saveLME, cmap, sorted_color_idx, transformId, uniqueID, removeOutlierParticipants);
...
save(fullfile(ResultsDir, [QuadBasename '_perceived_disparity_angle_elevation_results.mat']), ...
    'summaryStore', 'lmeStore', 'sorted_color_idx', 'mean_disparity_by_id', 'uniqueID', ...
    'removeOutlierParticipants', 'ElevationTransforms', 'version3DiskOnly');
