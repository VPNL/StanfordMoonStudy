function visualize_PM_VA_DS_EL_old(expDir,DataDir,QuadFile,task,saveLME,orientation,ResultsDir,logscale)
% function visualize_PM_VA_DS_EL_old(expDir,DataDir,QuadFile,task,saveLME,orientation)visualize_PM_VA_DS_EL
...
if strcmp(task,'Perceptual')
  title([ task ' $PM = 0.42 \cdot VA^{-0.083} \cdot D^{0.368} \cdot (1+E)^{0.12}$'], ...
      'Interpreter','latex','FontWeight','bold')
else

    title([task ' $PM = 0.52 \cdot VA^{-0.07} \cdot D^{0.14} \cdot (1+E)^{0.17}$'], ...
      'Interpreter','latex','FontWeight','bold')
end
