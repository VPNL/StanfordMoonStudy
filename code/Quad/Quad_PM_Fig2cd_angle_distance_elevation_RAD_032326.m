% Quad_PM_Fig2cd_angle_distance_elevation_RAD_0323226
%.
...
% set dirs
expDir='/Users/kalanit/Projects/PerceptualMagnification/Paper/ExperimentalData/QuadExperiments/'; % change path to match your computer
...
if topstick
    ...
    QuadFile = [QuadBasename '_topStick.csv'];
    QuadBasename = [erase(QuadFile,'.csv')] ;
else
    QuadFile='MatchingData0318.csv';
    QuadBasename = [erase(QuadFile,'.csv')] ;
...
if ObserverFlag % refer elevations and distances to observer line of sight
    all_quad_data.Elevation=all_quad_data.Observer_Elevation;
    all_quad_data.Distance=all_quad_data.Observer_Distance;
end
% % just take positive elevations
% positiveEL=find(all_quad_data.Elevation>0);
% all_data=all_quad_data;
% all_quad_data=all_quad_data(positiveEL,:);

%% 

ElevationTransform=1; % 1:2; 2: abs(e); 3: e/90; 4: abs(e)/90

if ElevationTransform==1
    ResultsDir=fullfile(expDir,'Paper_Fig2cd_032326_ElevationRad');
elseif  ElevationTransform==2
    ResultsDir=fullfile(expDir,'Paper_Fig2cd_032326_absElevationRad');
end
...
[Perceptual_lme_logPM_by_logAngle,Perceptual_lme_logPM_by_logDistance,Perceptual_lme_logPM_by_logElevation,...
    ...
    Perceptual_lme_logPM_by_logAngleNDistanceNElevation]=Quad_PM_by_task_rad(all_data_perceptual,Perceptual_tblName,ResultsDir,saveLME,cmap, sorted_idx,ElevationTransform);
...
[lme_test] = fit_PM_full_VA_D_Erad_model(all_data_perceptual,[Perceptual_tblName '_fullmodel_fit' task], ResultsDir, saveLME, cmap, sorted_idx,ElevationTransform);


VAmax=8;
distance_mm=500;
FigName=Perceptual_tblName;
visualize_real_perceived_predicted(all_data_perceptual,Perceptual_lme_logPM_by_logAngleNDistanceNElevation,visualizeDir,FigName,VAmax, distance_mm,0)
...
[Adjusted_lme_logPM_by_logAngle,Adjusted_lme_logPM_by_logDistance,Adjusted_lme_logPM_by_logElevation,...
    ...
    Adjusted_lme_logPM_by_logAngleNDistanceNElevation]=Quad_PM_by_task_rad(all_data_adjusted,Adjusted_tblName,ResultsDir,saveLME,cmap, sorted_idx, ElevationTransform);
...
[lme_test] = fit_PM_full_VA_D_Erad_model(all_data_adjusted,[Adjusted_tblName '_fullmodel_fit' task], ResultsDir, saveLME, cmap, sorted_idx,ElevationTransform);

VAmax=8;
distance_mm=500;
FigName=Adjusted_tblName;
visualize_real_perceived_predicted(all_data_adjusted,Adjusted_lme_logPM_by_logAngleNDistanceNElevation,visualizeDir,FigName,VAmax, distance_mm,ObserverFlag)
% 
