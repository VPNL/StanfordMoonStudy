function [lme_logPM_by_logAngle, lme_logPM_by_logElevation, ...
    lme_logPM_by_logAngleElevation] = ...
    Quad_PerceivedOffset_by_VAElevation(tbl, tblName, ResultsDir, ...
...
% QUAD_PERCEIVEDOFFSET_BY_VAELEVATION
% Fit interocular-offset visual-angle/elevation LME models and plot RI fits.
...
lme_logPM_by_logElevation = local_try_fitlme(tbl, elevationFormula); % 2
lme_logPM_by_logAngleElevation = local_try_fitlme(tbl, angleElevationFormula); % 3

if saveLME
    if ~exist(ResultsDir, 'dir')
        mkdir(ResultsDir);
    end
    savelmefile = fullfile(ResultsDir, [tblName '_VA_Elevation_RI_models.txt']);
    local_write_model_report(savelmefile, tbl, tblName, sourceCsv, ...
        modelTransform, ...
        {lme_logPM_by_logAngle, ...
        lme_logPM_by_logElevation, ...
        lme_logPM_by_logAngleElevation}, ...
        {angleFormula, elevationFormula, angleElevationFormula});
end
...
figFull = figure('Color', [1 1 1], 'Units', 'normalized', ...
    'Position', [0 0 .75 .80], 'Name', [tblName '_full_VA_E_RI'], ...
    'Visible', 'off');
tFull = tiledlayout(1,2,'Padding','compact','TileSpacing','loose');
tFull.Position = [0.09 0.18 0.78 0.70];
fullModelTitle = local_full_model_title(lme_logPM_by_logAngleElevation, ...
    modelTransform, nIDs);
...
local_plot_full_model_panel(axFullA, tbl, subjectcolor, ...
    lme_logPM_by_logAngleElevation, 'A', minDisp, maxDisp, ...
    modelTransform, true);
axFullE = nexttile;
local_plot_full_model_panel(axFullE, tbl, subjectcolor, ...
    lme_logPM_by_logAngleElevation, 'E', minDisp, maxDisp, ...
...
exportgraphics(figFull, fullfile(ResultsDir, [tblName '_full_VA_E_RI.png']), ...
    'Resolution', 600);
close(figFull);
end
...
reportOpts = struct();
reportOpts.ReportTitle = sprintf('Quad Interocular Offset h_va/Elevation LME Report: %s', ...
    char(string(tblName)));
reportOpts.GeneratedBy = mfilename;
reportOpts.SourceFile = sourceCsv;
reportOpts.ModelLabel = 'Interocular offset predicted by h_va and elevation';
reportOpts.SummaryLines = { ...
    ...
    sprintf('h_va parameter: h_va = visualangle(Width, distance).'), ...
...
comparisonPairs = { ...
    2, 3, 'Add h_va to elevation-only model'; ...
    1, 3, 'Add elevation to h_va-only model'};
...
reportTitle = sprintf('Quad Interocular Offset h_va/Elevation LME Report: %s', ...
    char(string(tblName)));
fprintf(fid, '%s\n', reportTitle);
...
function titleLines = local_full_model_title(lme, modelTransform, nIDs)
if nargin < 3
    nIDs = nan;
end
if isempty(lme)
...
modelLine = sprintf(['Full RI model: log_2(Offset) = b_0 + ' ...
    'b_{hVA}log_2(h_{VA}) + b_E%s + (1|ID)'], elevationLogTerm);
equationLine = sprintf('Offset = %s h_{VA}^{%s} %s^{%s}', ...
    local_format_title_number(scale), local_format_title_number(bVA), ...
    elevationTerm, local_format_title_number(bE));
statsLine = sprintf('p_{hVA}=%s, p_E=%s', ...
    local_format_pvalue(pVA), local_format_pvalue(pE));
interceptLine = sprintf('p_0=%s, n=%s', ...
    local_format_pvalue(p0), local_format_title_number(nIDs));
