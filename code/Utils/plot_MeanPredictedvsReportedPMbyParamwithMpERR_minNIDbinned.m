...
[paramBin, binStats, keepRowsForNID] = local_keep_bins_with_more_than_one_id(paramBin, binStats);
if ~all(keepRowsForNID)
    T = T(keepRowsForNID,:);
    itr = itr(keepRowsForNID);
    paramCol = paramCol(keepRowsForNID);
end
...
SignedPctErrLong = reshape(signedPctErrMat, [], 1);
SignedPctErr_ItrParam = table(IterationLong, BinLong, ModelLong, SignedPctErrLong, ...
...
if saveFlag
    reportModelLabels = predCols2modelLabel(predCols);
    [reportModel, reportModelLabel] = local_select_report_model(predCols, reportModelLabels);
    modelErrorSummaryTbl = local_build_model_error_summary_table(pctErrItrMean, predCols, reportModelLabels);
    binErrorReportTbl = local_build_bin_error_report_table(PctErr_ItrParam, SignedPctErr_ItrParam, binStats, reportModel, T);
    [p, f] = fileparts(char(saveFilename));
    reportFilename = fullfile(p, char(string(f) + "_bin_error_report.txt"));
    reportOpts = struct();
    reportOpts.Param = Param;
    reportOpts.minNID = minNID;
    reportOpts.didRebin = didRebin;
    reportOpts.rebinTolPct = rebinTolPct;
    reportOpts.figName = figName;
    reportOpts.reportModelLabel = reportModelLabel;
    reportOpts.generatedBy = 'plot_MeanPredictedvsReportedPMbyParamwithMpERR_minNIDbinned';
    write_bin_error_report(reportFilename, modelErrorSummaryTbl, binErrorReportTbl, reportOpts);
end
...
if strcmp(Param,'Distance')
    cmap=brighten(flipud(turbo(nParam)),0.4);
...
elseif strcmp(Param,'Real_Visual_Angle')
    cmap = purpleVioletBlueTurquoiseGreenColorMap(ceil(nParam));
else
...
markerSize = 15;
mainPlotFontSize = local_font_size_by_model_count(nModels, 11, 24);
mainTitleFontSize = max(16, 28);
errorPlotFontSize = local_font_size_by_model_count(nModels, 24, 24);
mainPlotLineWidth = local_style_by_model_count(nModels, 1, 2);
modelLabels = predCols2modelLabel(predCols);
for m = 1:nModels
    ax = nexttile;
    hold(ax, 'on');
    colormap(ax, cmap);
    clim(ax, [1 max(nParam, 2)]);

    % Reference line
    plot(ax, [minX maxX], [minX maxX], 'k:', 'LineWidth', mainPlotLineWidth);
    ...
    for p = 1:nParam
        ...
        if isfinite(dx) && dx > 0
            plot(ax, [xp-dx xp+dx], [yp yp], 'Color', colr, 'LineWidth', mainPlotLineWidth);
            plot(ax, [xp-dx xp-dx], [yp-cap yp+cap], 'Color', colr, 'LineWidth', mainPlotLineWidth);
            plot(ax, [xp+dx xp+dx], [yp-cap yp+cap], 'Color', colr, 'LineWidth', mainPlotLineWidth);
        end
        if isfinite(dy) && dy > 0
            plot(ax, [xp xp], [yp-dy yp+dy], 'Color', colr, 'LineWidth', mainPlotLineWidth);
            plot(ax, [xp-cap xp+cap], [yp-dy yp-dy], 'Color', colr, 'LineWidth', mainPlotLineWidth);
            plot(ax, [xp-cap xp+cap], [yp+dy yp+dy], 'Color', colr, 'LineWidth', mainPlotLineWidth);
        end
    end

    if nParam > 0
        markerColors = cmap(1:nParam,:);
        scatter(ax, xMean, yMean(:,m), markerSize, markerColors, 'filled');
    end
    ...
    % title(strrep(ttl, "_", "\\_"), 'Interpreter','tex','FontSize',12);
   
    ttl=modelLabels{m};
    ttl = erase(ttl, "predicted_PM_logPM_by_log");
    title(ax, strrep(ttl, "_", "\\_"), 'Interpreter','tex','FontSize',mainTitleFontSize);
end
...
for k = 1:numel(tickBins)
    ...
    if b == 1
        ...
        tickVal = binStats.NewParamMinValue(1);
    elseif b == nBins
        % Maximum Param value (from last bin)
        tickVal = binStats.NewParamMaxValue(nBins);
    else
        % Middle ticks: choose ONE of the following label styles

        % (A) Mean value of the bin:
        tickVal = binStats.NewParamMeanValue(b);
    ...
    if strcmp(Param,'Elevation')
        tickVal = local_param_display_values(tickVal, binStats.NewParamMaxValue, paramCol);
    end
    lab(k) = local_numeric_tick_label(tickVal);
end

cb.TickLabels = cellstr(lab);
cb.FontName = 'Avenir';
cb.FontSize = mainPlotFontSize;
...
% modelLabels = erase(modelLabels, "predicted_PM_logPM_by_log");
...
fh_pctbar = figure('Color','w','Name',pctFigName, 'Units','normalized','Position',[0.5 0.1 0.25 0.5]);
axb = axes(fh_pctbar);
hold(axb, 'on');
...
axb.FontSize = errorPlotFontSize;
...
if didRebin
    ylabel(axb, {'Mean % error', "Binned " + Param},'Interpreter','none');
else
    ylabel(axb, {'Mean % error', Param},'Interpreter','none');
end
...
function [reportModel, reportModelLabel] = local_select_report_model(predCols, modelLabels)
% Prefer the full angle+distance+elevation model for the concise bin report.
predCols = string(predCols);
modelLabels = string(modelLabels);
predLower = lower(predCols);
idx = find(contains(predLower, "loganglendistancenelevation"), 1, "first");
if isempty(idx)
    idx = numel(predCols);
end
reportModel = predCols(idx);
reportModelLabel = modelLabels(idx);
end

function summaryTbl = local_build_model_error_summary_table(pctErrItrMean, predCols, modelLabels)
% Summarize each tested model's mean absolute percent error across iterations.
nModels = numel(predCols);
modelLabels = string(modelLabels(:));
meanAbsPctError = nan(nModels, 1);
stdAbsPctError = nan(nModels, 1);
nIterations = nan(nModels, 1);

for ii = 1:nModels
    values = pctErrItrMean(:, ii);
    meanAbsPctError(ii) = mean(values, 'omitnan');
    stdAbsPctError(ii) = std(values, 'omitnan');
    nIterations(ii) = sum(isfinite(values));
end

summaryTbl = table(modelLabels, meanAbsPctError, stdAbsPctError, nIterations, ...
    'VariableNames', ["Model", "MeanAbsPctError", "StdAbsPctError", "N_Iterations"]);
summaryTbl = sortrows(summaryTbl, ["MeanAbsPctError", "StdAbsPctError", "Model"], {'ascend','ascend','ascend'});
end

function reportTbl = local_build_bin_error_report_table(PctErr_ItrParam, SignedPctErr_ItrParam, binStats, reportModel, T)
% Build one row per bin for one model, sorted from lowest to highest error.
...
vaMin = local_bin_stat_values(binStats, "Real_Visual_Angle_Min", binIdx);
vaMax = local_bin_stat_values(binStats, "Real_Visual_Angle_Max", binIdx);
dMin = local_bin_stat_values(binStats, "Distance_Min", binIdx);
dMax = local_bin_stat_values(binStats, "Distance_Max", binIdx);
eMin = local_bin_stat_values(binStats, "Elevation_Min", binIdx);
eMax = local_bin_stat_values(binStats, "Elevation_Max", binIdx);

if ismember("Elevation", string(T.Properties.VariableNames)) && ismember("Elevation_Max", string(binStats.Properties.VariableNames))
    eSource = T.Elevation;
    eMin = local_param_display_values(eMin, binStats.Elevation_Max, eSource);
    eMax = local_param_display_values(eMax, binStats.Elevation_Max, eSource);
end

vaRange = local_format_range(vaMin, vaMax, '%.2f');
dRange = local_format_range(dMin, dMax, '%.1f');
eRange = local_format_range(eMin, eMax, '%.1f');

reportTbl = table(binIdx, ...
    binStats.NewParamNID(binIdx), ...
...
function values = local_bin_stat_values(binStats, varName, binIdx)
if ismember(varName, string(binStats.Properties.VariableNames))
    values = binStats.(char(varName))(binIdx);
else
    values = nan(size(binIdx));
end
end

function fontSize = local_font_size_by_model_count(nModels, defaultFontSize, threeModelFontSize)
if nModels <= 3
    fontSize = threeModelFontSize;
else
    fontSize = defaultFontSize;
end
end

function styleValue = local_style_by_model_count(nModels, defaultValue, threeModelValue)
if nModels <= 3
    styleValue = threeModelValue;
else
    styleValue = defaultValue;
end
end

function [paramBinOut, binStatsOut, keepRows] = local_keep_bins_with_more_than_one_id(paramBin, binStats)
if ~ismember("NewParamNID", string(binStats.Properties.VariableNames))
    error("binStats must contain NewParamNID.");
end

validOldBins = find(binStats.NewParamNID > 1);
if isempty(validOldBins)
    error("No bins with more than 1 unique ID were available for plotting/reporting.");
end

keepRows = ismember(paramBin, validOldBins);
paramBinKept = paramBin(keepRows);

oldToNew = nan(max(paramBin), 1);
oldToNew(validOldBins) = 1:numel(validOldBins);
paramBinOut = oldToNew(paramBinKept);

binStatsOut = binStats(validOldBins,:);
binStatsOut.NewParamGroup = (1:height(binStatsOut))';
end
...
function valuesOut = local_param_display_values(valuesIn, maxValues, paramCol)
valuesOut = valuesIn;
if isempty(valuesIn)
    return
end

paramAbsMax = max(abs(paramCol), [], 'omitnan');
binAbsMax = max(abs(maxValues), [], 'omitnan');

% D90 transforms store elevation in units of E/90 or |E|/90. Convert these
% labels back into degrees for readability in the saved figures.
if isfinite(paramAbsMax) && isfinite(binAbsMax) && paramAbsMax <= 1.05 && binAbsMax <= 1.05
    valuesOut = 90 * valuesIn;
end
end
...
if ~isfinite(value)
    label = "";
elseif abs(value - round(value)) < 1e-6
