function [lme] = fit_PM_full_VA_D_Erad_model(tbl, tblName, ResultsDir, saveModels, mycolormap, sorted_idx, ElevationTransform,degreeFlag)
% fit_PM_full_VA_D_Erad_model
% Fit the full VA/D/E perceptual magnification LME using radian elevation
% transforms and plot model fits.
%
% Can use the shared elevation-transform codes used across the PM codebase
%   (3) ElevationRAD
%       log2(PM) ~ log2(VA) + log2(D) + log2(1+rad(E)) + (1|ID)
%   (4) absElevationRAD
%       log2(PM) ~ log2(VA) + log2(D) + log2(1+|rad(E)|) + (1|ID)
...
if nargin < 7 || isempty(ElevationTransform); ElevationTransform = 3; end
if nargin < 8 || isempty(degreeFlag); degreeFlag = 0; end
...
if ElevationTransform==3
  titlestr=sprintf('PM=%.2f(VA)^{%.2f}D^{%.2f}(1+E_{rad})^{%.2f}\n', 2.^Intercept,VAe,De,Ee);
...
local_plot_one_predictor_lme(nexttile, tbl, subjectcolor, lme, ...
    'log2real_angle', {'Visual Angle [deg]','log scale'}, muX, minPMLim, maxPMLim, true);

local_plot_one_predictor_lme(nexttile, tbl, subjectcolor, lme, ...
    'log2distance', {'Distance [m]', 'log scale'}, muX, minPMLim, maxPMLim, false);

if ElevationTransform==3
    if degreeFlag
        local_plot_one_predictor_lme(nexttile, tbl, subjectcolor, lme, ...
           'log2elevation', {'Elevation [degree]', 'log scale'}, muX, minPMLim, maxPMLim, true,degreeFlag);
    else
        local_plot_one_predictor_lme(nexttile, tbl, subjectcolor, lme, ...
           'log2elevation', {'Elevation [radian]', 'log scale'}, muX, minPMLim, maxPMLim, true,degreeFlag);
    end
elseif ElevationTransform==4
    if degreeFlag
    local_plot_one_predictor_lme(nexttile, tbl, subjectcolor, lme, ...
...
if ~ismember('log2elevation', tbl.Properties.VariableNames)
    if ismember('Elevation', tbl.Properties.VariableNames)
        if  ElevationTransform==3
            tbl.log2elevation = log2(1 + pi*(tbl.Elevation)/180); % l
        elseif ElevationTransform==4
            tbl.log2elevation = log2(1 + abs(pi*(tbl.Elevation)/180)); % 
        end
...
function local_plot_one_predictor_lme(ax, tbl, subjectcolor, lme, varName, xLabelStr, muX, minPMLim, maxPMLim, addMax,degreeFlag)
...
x = tbl.(varName);
[xt, xlbl, xlimVals] = local_pm_ticks(x, varName, degreeFlag);
...
function [xt, xlbl, xlimVals] = local_pm_ticks(x, varName, degreeFlag)
minX = min(x);
maxX = max(x);
...
switch varName
    ...
    otherwise
        ...
        nativeVals = (2.^xt) - 1;
        if degreeFlag
            xlbl = local_format_tick_labels(round(180 * nativeVals / pi, 1));
        else
            xlbl = local_format_tick_labels(round(nativeVals, 2));
        end
        nativeAll = (2.^x) - 1;
        span = max(nativeAll) - min(nativeAll);
        margin = max(0.05 * span, pi / 360);
