function lme = fit_PerceivedDisparity_by_angleElevation_lme(tbl, tblName, ResultsDir, saveModels, mycolormap, sorted_idx, modelTransform, fullUniqueID, removeOutlierParticipants)
% FIT_PERCEIVEDDISPARITY_BY_ANGLEELEVATION_LME
% Visual-angle/elevation-only mirror of fit_PerceivedDisparity_lme_wMax.
...
try
    lme = fitlme(tbl, 'log2mean_disparity ~ 1 + log2real_visual_angle + log2elevation + (1|ID)');
catch ME
...
if modelTransform == 2
    titlestr = sprintf('Disparity=%.2f(VA)^{%.2f}(1+|E|)^{%.2f}', 2.^Intercept, VAe, Ee);
elseif modelTransform == 6
    titlestr = sprintf('Disparity=%.2f(VA)^{%.2f}(1+|E|/90)^{%.2f}', 2.^Intercept, VAe, Ee);
else
    titlestr = sprintf('Disparity=%.2f(VA)^{%.2f}(1+E/90)^{%.2f}', 2.^Intercept, VAe, Ee);
end
...
fprintf(fid, 'Model: log2mean_disparity ~ 1 + log2real_visual_angle + log2elevation + (1|ID)\n');
fprintf(fid, 'Skipped: %s\n\n', msg);
end

muX = [mean(tbl.log2real_visual_angle,'omitnan'), mean(tbl.log2elevation,'omitnan')];
...
X = [tbl.log2real_visual_angle, tbl.log2elevation];
...
fprintf(fid, 'Design rank [1 VA E]: %d of %d\n\n', rank([ones(size(X,1),1) X]), size(X,2) + 1);

if size(X,1) >= 2
    C = corrcoef(X);
    fprintf(fid, 'Predictor correlation matrix [VA E]:\n');
    for i = 1:size(C,1)
        fprintf(fid, '%.4f\t%.4f\n', C(i,1), C(i,2));
    end
else
