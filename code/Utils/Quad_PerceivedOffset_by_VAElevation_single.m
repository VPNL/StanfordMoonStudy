function [lme_logPM_by_logVisualAngle, lme_logPM_by_logVisualAngle_RS, ...
    ...
    Quad_PerceivedOffset_by_VAElevation_single(tbl, tblName, ...
...
% QUAD_PERCEIVEDOFFSET_BY_VAELEVATION_SINGLE
% Fit and plot single-factor visual-angle/elevation perceived-offset models
...
if runStereoAnalysis
    ...
    stereoGroupSummaryLines = {'StereoGroup interaction models: skipped by caller.'};
end
...
if saveLME
    savelmefile = fullfile(ResultsDir, [tblName '_VA_Elevation_single_RI_RS.txt']);
...
figRIVDE = figure('Color', [1 1 1], 'Units', 'normalized', 'Position', [0 0 .75 .72], 'Name', [tblName '_VA_E_RI'], 'Visible', 'off');
tRIVDE = tiledlayout(1,2,'Padding','compact','TileSpacing','compact');
tRIVDE.Position = [0.09 0.12 0.78 0.64];
axRIVA = nexttile;
...
local_add_subject_colorbar(figRIVDE, mycolormap, nIDs, colorbarLabel, ...
    colorConfig, axRIE);
exportgraphics(figRIVDE, fullfile(ResultsDir, [tblName '_VA_E_RI.png']), 'Resolution', 600);
close(figRIVDE);

figRSVDE = figure('Color', [1 1 1], 'Units', 'normalized', 'Position', [0 0 .75 .72], 'Name', [tblName '_VA_E_RS'], 'Visible', 'off');
tRSVDE = tiledlayout(1,2,'Padding','compact','TileSpacing','compact');
tRSVDE.Position = [0.09 0.12 0.78 0.64];
axRSVA = nexttile;
...
local_add_subject_colorbar(figRSVDE, mycolormap, nIDs, colorbarLabel, ...
    colorConfig, axRSE);
exportgraphics(figRSVDE, fullfile(ResultsDir, [tblName '_VA_E_RS.png']), 'Resolution', 600);
close(figRSVDE);
end

function local_write_model_report(reportFile, tbl, tblName, sourceCsv, ...
    modelTransform, stereoGroupSummaryLines, models, formulas)
...
reportOpts.ModelLabel = 'Single-factor interocular offset models by h_va and elevation';
baseSummaryLines = { ...
...
comparisonPairs = { ...
    ...
    3, 4, 'Elevation RI vs RS comparison'; ...
    1, 5, 'h_va StereoGroup RI interaction comparison'; ...
    3, 6, 'Elevation StereoGroup RI interaction comparison'};
