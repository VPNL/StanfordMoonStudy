...
% Shapes are placed left-to-right with fixed gaps and aligned to a shared
% bottom baseline.
baselineY_mm = imgHeight_mm - gap_mm;
...
baselineY_px = baselineY_mm * resolution_pixels_per_mm;
...
for i = 1:3
    text(cx(i), yLabel_pixels, labels{i}, ...
        ...
        'Color','k', 'FontName','Avenir', 'FontSize', 16);
end
...
text(xBL_pixels, yBL_pixels, textstring, ...
    ...
    'Color','k', 'FontName','Avenir', 'FontSize', 10);

% ----------------------------- Save -------------------------------------
if saveFlag
    exportgraphics(fh, saveFilename, 'Resolution', 150);
end
