function lme = fit_PerceivedDisparity_lme_wMax(tbl, tblName, ResultsDir, saveModels, mycolormap, sorted_idx, modelTransform, fullUniqueID, removeOutlierParticipants)
% FIT_PERCEIVEDDISPARITY_LME_WMAX
% Mirror of fit_PM_full_VA_D_E_model for mean perceived disparity.
...
try
    lme = fitlme(tbl, 'log2mean_disparity ~ 1 + log2real_visual_angle + log2distance + log2elevation + (1|ID)');
catch ME
...
if modelTransform == 2
    titlestr = sprintf('Disparity=%.2f(VA)^{%.2f}D^{%.2f}(1+|E|)^{%.2f}', 2.^Intercept, VAe, De, Ee);
elseif modelTransform == 6
    titlestr = sprintf('Disparity=%.2f(VA)^{%.2f}D^{%.2f}(1+|E|/90)^{%.2f}', 2.^Intercept, VAe, De, Ee);
else
    titlestr = sprintf('Disparity=%.2f(VA)^{%.2f}D^{%.2f}(1+E/90)^{%.2f}', 2.^Intercept, VAe, De, Ee);
end
...
fprintf(fid, 'Model: log2mean_disparity ~ 1 + log2real_visual_angle + log2distance + log2elevation + (1|ID)\n');
fprintf(fid, 'Skipped: %s\n\n', msg);
end

muX = [mean(tbl.log2real_visual_angle,'omitnan'), mean(tbl.log2distance,'omitnan'), mean(tbl.log2elevation,'omitnan')];
minDisp = max(0.01, min(tbl.MeanDisparity));
maxDisp = max(tbl.MeanDisparity);

fig1 = figure('Color',[1 1 1],'Units','normalized','Position',[0 0 1 .6],'Name',tblName,'Visible','off');
tiledlayout(1,3,'Padding','compact','TileSpacing','compact');
local_plot_one_predictor(nexttile, tbl, subjectcolor, lme, 'log2real_visual_angle', {'Visual Angle [deg]','log scale'}, muX, minDisp, maxDisp, modelTransform);
...
X = [tbl.log2real_visual_angle, tbl.log2distance, tbl.log2elevation];
...
fprintf(fid, 'Design rank [1 VA D E]: %d of %d\n\n', rank([ones(size(X,1),1) X]), size(X,2) + 1);

if size(X,1) >= 2
    C = corrcoef(X);
    fprintf(fid, 'Predictor correlation matrix [VA D E]:\n');
    for i = 1:size(C,1)
...
switch varName
    case 'log2real_visual_angle'
        x = tbl.log2real_visual_angle;
        xData = tbl.Real_Visual_Angle;
        varyIdx = 1;
    case 'log2distance'
        x = tbl.log2distance;
        xData = tbl.Distance;
