...
% visualize real, perceived, predicted VA
VAmax=1;
distance_mm=500;
minUniqueIDs=30;
FigName=[combined_basename '_fullmodel_' task];
moonStudy=find(strcmp(all_data_perceptual.Study,'MoonStudy'));
moon_data_perceptual=all_data_perceptual(moonStudy,:);
quadStudy=find(strcmp(all_data_perceptual.Study,'QuadStudy'));
quad_data_perceptual=all_data_perceptual(quadStudy,:)
Param='Elevation';
summaryTbl = visualize_real_perceived_predicted_binned(moon_data_perceptual, Combined_Perceptual_lme_logPM_by_logAngleNDistanceNElevation,...
    ResultsDir, FigName, Param, minUniqueIDs, VAmax, distance_mm);
Param='Real_Visual_Angle';
summaryTbl = visualize_real_perceived_predicted_binned(quad_data_perceptual, Combined_Perceptual_lme_logPM_by_logAngleNDistanceNElevation,...
    ResultsDir, FigName, Param, minUniqueIDs, VAmax, distance_mm);
...
[Combined_Adjusted_lme_logPM_by_logAngle,Combined_Adjusted_lme_logPM_by_logDistance,Combined_Adjusted_lme_logPM_by_logElevation,...
    ...
    Combined_Adjusted_lme_logPM_by_logAngleNDistanceNElevation]=Quad_PM_by_task(all_data_adjusted,[combined_basename '_' task],ResultsDir,saveLME,cmap_combined, sorted_idx);
...
[lme_test] = fit_PM_lme(all_data_adjusted,[combined_basename '_fullmodel_' task], ResultsDir, saveLME, cmap_combined, sorted_idx);
 
VAmax=1;
distance_mm=500;
minUniqueIDs=30;
FigName=[combined_basename '_fullmodel_' task];
% separate table to moon and quad data so we can separately look at
% predictions and error patterns

moonStudy=find(strcmp(all_data_adjusted.Study,'MoonStudy'));
moon_data_adjusted=all_data_adjusted(moonStudy,:);
quadStudy=find(strcmp(all_data_adjusted.Study,'QuadStudy'));
quad_data_adjusted=all_data_adjusted(quadStudy,:);
Param='Elevation';
summaryTbl = visualize_real_perceived_predicted_binned(moon_data_adjusted, Combined_Adjusted_lme_logPM_by_logAngleNDistanceNElevation,...
    ResultsDir, FigName, Param, minUniqueIDs, VAmax, distance_mm);
Param='Real_Visual_Angle';
summaryTbl = visualize_real_perceived_predicted_binned(quad_data_adjusted, Combined_Adjusted_lme_logPM_by_logAngleNDistanceNElevation,...
    ResultsDir, FigName, Param, minUniqueIDs, VAmax, distance_mm);
