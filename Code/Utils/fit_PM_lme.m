function [lme] = fit_PM_lme(tbl, tblName, ResultsDir, saveModels, mycolormap, sorted_idx)
% fit_PM_lme
%
% Fits:
...
local_plot_one_predictor_lme(nexttile, tbl, subjectcolor, lme, ...
    'log2real_angle', {'Visual Angle [deg]','log scale'}, muX, minPMLim, maxPMLim);

local_plot_one_predictor_lme(nexttile, tbl, subjectcolor, lme, ...
    'log2distance', {'Distance [m]', 'log scale'}, muX, minPMLim, maxPMLim);

local_plot_one_predictor_lme(nexttile, tbl, subjectcolor, lme, ...
    'log2elevation', {'Elevation [deg]', 'log scale'}, muX, minPMLim, maxPMLim);
...
%mm = get(ax,'XTick');
% Standardize x ticks to fixed log2 steps (1 = doubling)
% and convert tick labels back to native units for display.
xmin = min(xgrid);
xmax = max(xgrid);
xt = floor(xmin):1:ceil(xmax);
