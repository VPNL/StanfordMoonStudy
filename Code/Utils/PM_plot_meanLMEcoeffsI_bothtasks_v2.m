function [statsTblP,statsTblA,fh] = PM_plot_meanLMEcoeffsI_bothtasks_v2(perceptual_summaryTbl,adjusted_summaryTbl,saveFilename,coefVars,verboseFlag)
...
subplot(1,2,1)
plotPackedBars(muP(:,coefCols), sigP(:,coefCols), modelListP, coefExp, colorsExp);
title('Perceptual','FontSize',20);
ylabel('Exponent','FontSize',20);

subplot(1,2,2)
plotPackedBars(muA(:,coefCols), sigA(:,coefCols), modelListA, coefExp, colorsExp);
title('Adjusted','FontSize',20);
ylabel('Exponent','FontSize',20);
...
nModels = size(mu,1);
barW    = 0.18;            % fixed thin width (as if always 3 coefficients)
slotDelta = barW * 1.25;   % fixed slot spacing; avoids overlap
...
set(ax,'FontName','Avenir','FontSize',16);


l=legend(coefNames, 'Interpreter','none', 'Location','bestoutside', 'box','off', 'FontSize',14);
end
