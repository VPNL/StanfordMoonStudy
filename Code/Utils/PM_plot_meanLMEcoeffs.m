...
% The order of the coefficients in the models is
% intercept
% visual angle (if exists)
% distance     (if exists)
% elevation    (if exists)  
...
modelLabel = modelList2modelLabel(modelList);


ax.XTick = 1:numel(modelLabel);
ax.XTickLabel = modelLabel;
...
legend(hLeg, legendNames, 'Interpreter','none', 'Location','bestoutside','box','off','FontSize',24);
...
outTbl = table(modelList, modelFormula, 'VariableNames', {'modelName','FormulaStr'});

for k = 1:numel(coefVars)
    baseName = string(coefVars{k});
    meanName = matlab.lang.makeValidName(baseName + "_mean");
    stdName  = matlab.lang.makeValidName(baseName + "_std");

    outTbl.(meanName) = mu(:,k);
    outTbl.(stdName)  = sig(:,k);
end
if verboseFlag
    disp(outTbl);
end
