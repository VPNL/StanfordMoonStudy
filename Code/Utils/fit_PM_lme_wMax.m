function [lme] = fit_PM_lme_wMax(tbl, tblName, ResultsDir, saveModels, mycolormap, sorted_idx)
% fit_PM_lme
%
% Fits:
%   (1) linear mixed-effects model (fitlme):
...
% -------------- FIGURE 1: LME --------------
fig1 = figure('Color',[1 1 1],'Units','normalized','Position',[0 0 1 .6],'Name',tblName);
...
local_plot_one_predictor_lme(nexttile, tbl, subjectcolor, lme, ...
    'log2elevation', {'Elevation [deg]', 'log scale'}, muX, minPMLim, maxPMLim, true);
...
function local_plot_one_predictor_lme(ax, tbl, subjectcolor, lme, varName, xLabelStr, muX, minPMLim, maxPMLim, addMax)
...
%     xgrid = linspace(0.95*min(x)-1, max(x)-1, 200)'
% else
    xgrid = linspace(0.95*min(x), max(x), 200)';
% end
...
fill([xgrid; flipud(xgrid)], [yCI(:,1); flipud(yCI(:,2))], 1, ...
	...
	% Standardize x ticks to fixed log2 steps (1 = doubling) and always include
	% a final tick at the maximal x value (even if it is not an integer log2 step).
	xmin = min(xgrid);
	xmax = max(xgrid);
	set(ax,'XLim',[xmin xmax]);
	
	% Base ticks at integer log2 steps within the axis range
	xt = floor(xmin):1:floor(xmax);
	
	% Optionally add a final tick exactly at xmax (even if it is not an integer log2 step)
	if addMax
	    if isempty(xt)
	        xt = xmax;
	    elseif abs(xmax - xt(end)) > 1e-10
	        xt = [xt xmax];
	    end
	end
	
	% Ensure monotonic, unique ticks
	xt = unique(xt,'stable');
	set(ax,'XTick',xt);
	if strcmp(varName,'log2elevation')
	    xlbl = (2.^xt) - 1; % since log2elevation = log2(1 + Elevation)
	else
	    xlbl = 2.^xt;
	end
