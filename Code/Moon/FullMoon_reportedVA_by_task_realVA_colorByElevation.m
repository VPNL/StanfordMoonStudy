...
lme_reportedVA_by_VA_Elevation_i = fitlme(all_data,'Reported_Visual_Angle ~ Real_Visual_Angle*Elevation + (1|ID)'); % as log function doesn't deal with 0 and elevation can be 0 add 1 as a regularization factor

% 
model_comp=compare(lme_reportedVA_by_VA_Elevation,lme_reportedVA_by_VA_Elevation_i);
if saveLME
   ...
   lme_reportedVA_by_VA_Elevation_i
...
%
figh=figure('Color',[1 1 1],'Units','normalized','Position',[0 0 1 1],'Name',[basename 'Mppm Reported Visual Angle vs Real Visual Angle'])
%subplot(1,3,1); 
% hold on 
% plot fixed effect
...
plot([min(all_data.Real_Visual_Angle) max(all_data.Real_Visual_Angle)],[min(all_data.Real_Visual_Angle) max(all_data.Real_Visual_Angle)] ,'Color',[.8 .8 .8],'LineWidth',3)
axis('square')
title(titlestr)
