function [lme_logPM_by_logAngle,lme_logPM_by_logDistance,lme_logPM_by_logElevation,...
    ...
    lme_logPM_by_logAngleNDistanceNElevation]=Quad_PM_by_task(tbl,tblName,ResultsDir,saveLME,colormap, sorted_idx)
...
minElevation=min(tbl.nElevation);
maxElevation=max(tbl.Elevation);
fprintf(1,'Elevation [ %.2f _%.2f]\n', minElevation, maxElevation)
...
tbl.log2nelevation=log2(-tbl.nElevation+1); % add a regularization term and minus elevation so that the log won't explode for an elevation of 0
...
lme_logPM_by_logElevation= fitlme((tbl),'log2ratio_visual_angle ~ log2elevation + log2nelevation + (1| ID)');

lme_logPM_by_logAngleNDistance= fitlme(tbl,'log2ratio_visual_angle ~ log2real_visual_angle + log2distance + (1| ID)');
lme_logPM_by_logAngleNElevation= fitlme(tbl,'log2ratio_visual_angle ~ log2real_visual_angle + log2elevation + log2nelevation+ (1| ID)');
lme_logPM_by_logDistanceNElevation= fitlme(tbl,'log2ratio_visual_angle ~ log2distance + log2elevation + + log2nelevation + (1| ID)');

lme_logPM_by_logAngleNDistanceNElevation= fitlme(tbl,'log2ratio_visual_angle ~ log2real_visual_angle + log2distance + log2elevation+ + log2nelevation + (1| ID)');
if saveLME
...
%set(gca,'XTick',log2(minRealAngle):tickdelta:log2(maxRealAngle),'XTickLabel',round(2.^[log2(minRealAngle):tickdelta:log2(maxRealAngle)],1),'XTickLabelRotation',90);

set(gca,'XTick',log2(minRealAngle):tickdelta:log2(maxRealAngle),'XTickLabel',round(2.^[log2(minRealAngle):tickdelta:log2(maxRealAngle)],1));
...
%set(gca,'XTick',log2(minDistance):tickdelta:log2(maxDistance),'XTickLabel', round(2.^[log2(minDistance):tickdelta:log2(maxDistance)],1),'XTickLabelRotation',90);
set(gca,'XTick',log2(minDistance):tickdelta:log2(maxDistance),'XTickLabel', round(2.^[log2(minDistance):tickdelta:log2(maxDistance)],0));
set(gca,'YTick',log2(minPMLim):1:log2(maxPMLim) ,'YTickLabel',2.^([log2(minPMLim) :1: ceil(log2(maxPMLim))]));
...
% get upper and lower coeffiecents
mean_intercept_elevationL=lme_logPM_by_logElevation.Coefficients.Lower(1)
mean_slope_elevationL=lme_logPM_by_logElevation.Coefficients.Lower(3)
mean_intercept_elevationU=lme_logPM_by_logElevation.Coefficients.Upper(1)
mean_slope_elevationU=lme_logPM_by_logElevation.Coefficients.Upper(3)

minlognElevation=min(tbl.nlog2elevation);
maxlognElevation=max(tbl.mlog2elevation);
xlinerange=minlognElevation:.05:maxnlogElevation; 
ylinerange=zeros(size(xlinerange));
...
scatter(tbl.nlog2elevation, tbl.log2ratio_visual_angle, markerSize, subjectcolor, 'filled');
plot (xlinerange, ylinerange,'k:','LineWidth',1);% plot no magnification line
plot (xlinerange, mean_intercept_elevation+mean_slope_elevation*xlinerange,'k-','LineWidth',3); % fixed effects linear estimate
fill(xvector, yvector, 1,'facecolor', 'k', 'edgecolor', 'none', 'facealpha', 0.4); % confidence interval on fixed effect
