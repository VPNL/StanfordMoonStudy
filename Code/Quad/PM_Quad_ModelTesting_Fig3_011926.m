...
ResultsDir='/Users/kalanit/Projects/PerceptualMagnification/Paper/ExperimentalData/QuadExperiments/Paper_Fig3_Modeltesting_012226';
...
for t=1:nTasks
    ...
    % %  plot mean predicted vs reported PM by parameter ----------
    % 
    % for p=1:numel(paramsToPlot)
    %     Param = paramsToPlot{p};
    %     saveFilename = fullfile(ResultsDir, sprintf('%s_%diterations_%s_%s.png', figName, nIterations, Param, QuadBasename));
    %     [SummaryItrParam, SummaryParam, MSE_ItrParam, MSE_Itr, fh, fh_violin, PctErr_ItrParam, PctErr_Itr, fh_pctbar] = ...
    %         plot_MeanPredictedvsReportedPMbyParamwithMpERR(S.(task).all_testing_Tbl, Param, saveFilename, figName, 'sd');
    % end
...
%% Evaluated on binned  paramter values 
...
for t=1:nTasks % loop on tasks
    ...
    if strcmp(task,'Perceptual')
        ...
        suffixTemplate = '%s_%diterations_%s_%s';
    else % adjusted
        figName = sprintf('Adjusted_%dminNID', minNID);
        suffixTemplate = '%s_%diterations_%s_combined_%s';
    end

    for p=1:numel(paramsToPlot) % loop on parameters to plot
        Param = paramsToPlot{p};
        saveBase = sprintf(suffixTemplate, figName, nIterations, Param, QuadBasename);
        ...
        [SummaryItrParambinned, SummaryParambinned, MSE_ItrParam, MSE_Itr, fh, fh_violin, PctErr_ItrParam, PctErr_Itr, fh_pctbar] = ...
            plot_MeanPredictedvsReportedPMbyParamwithMpERR_minNID(S.(task).all_testing_Tbl, Param, minNID, saveFilename, figName, 'sd');
    end
end
%
close all
% save results
saveFilename = fullfile(ResultsDir, sprintf('%diterations_lme_coeffs_%s.mat', nIterations, QuadBasename));
save(saveFilename);