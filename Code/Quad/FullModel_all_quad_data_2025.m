...
for i=nTasks:-1:1
    ...
    if strcmp(allTasks(i),'Adjusted') % adjusted task
        ...
        if saveLME % save stats table
             savelmefile=fullfile('.','Results', basename, [basename '_lme_reported_vs_real_angle_adjusted.txt']);
             diary(savelmefile)
        ...
        if saveLME 
            savelmefile=fullfile('.','Results', basename, [basename '_lme_reported_vs_real_angle_estimated.txt']);
            diary(savelmefile)
    ...
    subplot(1,2,2-i+1); hold on 
    ID=all_data.ID(task_i);
    cmap=jet(nsubjects);
    clear subjectcolor;
    xvectorS= [0:maxRealAngle+1];
    if(strcmp(allTasks(i),'Perceptual'))
       %setcolors
       for c=1:length(ID)
    ...
    plot (0:maxAngle, 0:maxAngle,'k:','LineWidth',3);
    plot (xvector, mean_slope*xvector,'k-','LineWidth',3);
    fill(xvector, yvector, 1,'facecolor', 'k', 'edgecolor', 'none', 'facealpha', 0.3);
...
% filenameEPS=fullfile('.','Results',basename, [basename , '_', num2str(nsubjects),'.eps']);
% exportgraphics(figh,filenameEPS,'Resolution',600,'ContentType','vector'); %% vector graphics

filenamePNG=fullfile('.','Results',basename, [basename,'_', num2str(nsubjects),'.png']);
...
if saveLME % save stats table
     savelmefile=fullfile('.','Results', basename, [basename '_lme_reported_angle_vs_angle_and_task.txt']);
     diary(savelmefile)
     lme_by_angleNtask
     diary off
end
...
for i=1:nTasks
   ...
   % get individual subject slopes
    ...
    if strcmp(allTasks(i),'Adjusted')
       ...
       if saveLME
            savelmefile=fullfile('.','Results', basename, [basename '_lme_logPM_by_logangle_adjusted.txt']);
            diary(savelmefile)
       ...
       if saveLME
            savelmefile=fullfile('.','Results', basename, [basename '_lme_logPM_by_logangle_estimated.txt']);
            diary(savelmefile)
            lme_logPM_by_logangle
            diary off
       end     
    ...
    if strcmp(allTasks(i),'Adjusted')
       ...
       if saveLME
            savelmefile=fullfile('.','Results', basename, [basename '_lme_logPM_by_logdistance_adjusted.txt']);
            diary(savelmefile)
       ...
       if saveLME
            savelmefile=fullfile('.','Results', basename, [basename '_lme_logPM_by_logdistance_estimated.txt']);
            diary(savelmefile)
            lme_logPM_by_logdistance
            diary off
       end      
    ...
    % filenameEPS= fullfile('.','Results',basename, basename, [basename '_logAxis_', allTasks{i} ,'_', num2str(nsubjects),'.eps']);
    % exportgraphics(figh,filenameEPS,'Resolution',600,'ContentType','vector');

    filenamePNG=fullfile('.','Results', basename, [basename,'_logAxis_', allTasks{i} ,'_', num2str(nsubjects),'.png']);
    exportgraphics(figh,filenamePNG,'Resolution',600);
    ...
    if strcmp(allTasks(i),'Adjusted')
       ...
       if saveLME
            savelmefile=fullfile('.','Results', basename,[basename '_lme_logPM_by_logangle_and_logdistance_adjusted.txt']);
            diary(savelmefile)
       ...
       if saveLME
            savelmefile=fullfile('.','Results', basename, [basename '_lme_logPM_by_logangle_and_logdistance_estimated.txt']);
            diary(savelmefile)
            lme_logPM_by_logangle_and_logdistance
            diary off
       end  
    ...
    % 
    % % generate range to evaluate function
    % minAngle=min(all_data.Real_Visual_Angle);
    % maxAngle=max(all_data.Real_Visual_Angle);
    % log2_angleRange=linspace(round(log2(minAngle)),round(log2(maxAngle))); 
    % minDistance=min(all_data.Distance);
    % maxDistance=max(all_data.Distance);
    % log2_distanceRange=linspace(round(log2(minDistance)),round(log2(maxDistance)));
    % for d=1:length(log2_distanceRange)
    %     for a=1:length(log2_angleRange)
    %         Ypred(d,a)=intercept + beta_log_real_angle * log2_angleRange(a) + beta_log_distance * log2_distanceRange(d);
    %     end
    % end
    % % [mesh_real_angle, mesh_distance] = meshgrid(log2_angleRange, log2_distanceRange);
    % %Y_pred = intercept + beta_log_real_angle * mesh_real_angle + beta_log_distance * mesh_distance;
    % imagesc(Y_pred); colorbar; axis('square');
    % set(gca,'XTick',log2(.25):1:log2(8),'XTickLabel',2.^[log2(.25):1:log2(8)]);
    % 
    % ylabel ('Distance (m) log scale')
    % xlabel ('Visual Angle (deg) log scale')
    %  titlestr=sprintf('PM=%-.2f*va^-.2f*D-.2f\n p=%-.2e \n intercept=%-.2f, p=%-.2e',...
    %      string(intercept, beta_log_real_angle,beta_log_distance, ...
    %      fe_stats.pValue(1), fe_stats.pValue(2), fe_stats.pValue(3));
    % title(titlestr)
    % set(gca,'FontSize',18)
...
for i=1:nTasks
   ...
   % get regression coefficients and confidence interval on fixed effect
    ...
    % save figure
    % filenameEPS=fullfile('.','Results',basename, [basename '_', allTasks{i} ,'_', num2str(nsubjects),'.eps']);
    % exportgraphics(figh,filenameEPS,'Resolution',600,'ContentType','vector'); %% vector graphics
    % 
    filenamePNG=fullfile('.','Results',basename, [basename,'_', allTasks{i} ,'_', num2str(nsubjects),'.png']);
    exportgraphics(figh,filenamePNG,'Resolution',600);
end
...
if saveLME % save stats table
    savelmefile=fullfile('.','Results', basename,[basename '_JointvsSingle_model_comparison.txt']);
    diary(savelmefile)
    fprintf(1,'Estimated task\n')
    compare(lme_logPM_by_logangle_estimated,     lme_logPM_by_logangle_and_logdistance_estimated)   
    compare(lme_logPM_by_logdistance_estimated  ,lme_logPM_by_logangle_and_logdistance_estimated)

    fprintf(1,'Adjusted task\n')
    compare(lme_logPM_by_logangle_adjusted,     lme_logPM_by_logangle_and_logdistance_adjusted)
    compare(lme_logPM_by_logdistance_adjusted  ,lme_logPM_by_logangle_and_logdistance_adjusted)
   
    diary off
end

%%
% remake the prior figures adding a 5th panel showing an image of estimated perceptual magnification vs distance (x) and angle (y)
% where the color of the cell in the matrix is the estimated PM
% magnifcation from joint model
% calculate separate for each task as each task has its own model
%
