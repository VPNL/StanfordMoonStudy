...
if saveLME % save stats table
     savelmefile=fullfile('.', ResultsDir, [basename '_lme_moon_visualangle_by_altitudeNtask.txt']);
     diary(savelmefile)
...
if saveLME % save stats table
     savelmefile=fullfile('.',ResultsDir, [basename '_lme_moon_ratio_by_altitudeNtask.txt']);
     diary(savelmefile)
...
vars2keep = {'ID','Altitude','Task','Ratio_Visual_Angle'};
...
vars2keep = {'ID','Altitude','Task','Reported_Visual_Angle'};
tmp = all_data(:,vars2keep);
tmp.Task = categorical(tmp.Task);
...
xlabel ('Adjusted Reported Angle (degrees)')
ylabel ('Perceptual Visual Angle (degrees)') 
set(gca,'FontSize',28, 'FontName','Avenir')
xlim([0 max(data_rearrangedAngle.Perceptual)])
ylim([0 max(data_rearrangedAngle.Perceptual)])
axis('square')
titlestr=sprintf('Visual angle across tasks\n correlation=%5.2f p=%5.2e n=%d',rAngle,pAngle, nsubjects);
title(titlestr)
...
axis('square')
titlestr=sprintf('Perceptual Magnification across tasks\n correlation=%5.2f p=%5.2e n=%d',rPM,pPM, nsubjects);
title(titlestr)
