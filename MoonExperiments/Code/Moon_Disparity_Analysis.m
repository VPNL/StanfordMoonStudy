%% to do
% change to function
% make a plot that shows disparity vs elevation and then PM for perceptual
% case vs Disparity and PM for adjusted case vs Disparity
% plot pm magnification vs elevation only for the paricipants who have
% disparity;
...
allowed_dates = {  'Jan 12th', 'Jan 13th', 'May 13th' 'Aug 7th' 'Aug 9th'};  %  allowed dates 'Nov 15th',
...
% use sorted colormap from Fig1
LoadDir='PaperFig1';

loadfile=fullfile(dataDir, LoadDir, [basename '_sortedidx']);
load(loadfile);
% set colormap
clear subjectcolor;
cmap=jet(nsubjects);
...
if saveLME % save stats table
     savelmefile=fullfile('.',ResultsDir, [basename '_' task '_lme_disparity_by_altitude.txt']);
     diary(savelmefile)
...
if pvalDis<0.001 % nice writing of p-values
...
disparity_data = disparity_data(ismember(disparity_data.Date, allowed_dates), :);
% Reuse the SAME uniqueIDD, sorted_idx, and cmap defined earlier
IDD2 = disparity_data.ID;
subjectcolor2 = zeros(length(IDD2),3);
for c = 1:length(IDD2)
    cindex = find(uniqueIDD == IDD2(c));          % index of this subject within uniqueIDD
    sorted_cindex = find(sorted_idx == cindex);    % map to sorted color index (same as left plot)
    subjectcolor2(c,:) = cmap(sorted_cindex,:);    % assign the matching color
end
% ---------------------------------------------------------------------
...
if saveLME % save stats table
     savelmefile=fullfile('.',ResultsDir, [basename '_' task '_lme_ratio_by_disparity.txt']);
     diary(savelmefile)
...
%scatter(disparity_data.Disparity_VA,disparity_data.Ratio_Visual_Angle,'o','filled');
...
ylim([0 maxRatio])
set(gca,'FontSize',24,'FontName','Avenir')
if pvalD<0.001 % nice printing of pvalues
    titlestr=sprintf('%s \n intercept=%3.2f \n  slope=%3.2f p=%5.2e \n n=%d',task,interceptD,slopeD,pvalD ,length(unique(IDD)));
else
    titlestr=sprintf('%s \n intercept=%3.2f \n  slope=%3.2f p=%.3f \n n=%d',task,interceptD,slopeD,pvalD ,length(unique(IDD)));
end
...
filenamePNG=fullfile(dataDir,ResultsDir,[basename,'_' task ,'_', num2str(nsubjectsD),'.png']);
