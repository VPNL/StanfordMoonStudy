function [feEfxR,feNamesR,festatsR,reEfxR,reNamesR,reStatsR] = FullMoon_ratioVrelativedisparity(dataDir, datafile,task, ResultsDir, saveLME, subplotNum, sorteduniqueIDD)
% 
% 
...
% adjust disparity measure by size of moon as we took measurements that include the moon
...
maxRelativeDisparity=max(disparity_data.RelativeDisparity);
minRelativeDisparity=min(disparity_data.RelativeDisparity);
...
%%
lme_ratio_by_disparity = fitlme(disparity_data,'Ratio_Visual_Angle~RelativeDisparity + (1|ID)');
lme_ratio_by_disparity_RS = fitlme(disparity_data,'Ratio_Visual_Angle~RelativeDisparity + (RelativeDisparity|ID)');
...
disp('PM as a function of relative disparity: random intercepts and slopes vs random intercepts comparison')
...
lme_ratio_by_disparity_Elevation_RS = fitlme(disparity_data,'Ratio_Visual_Angle~RelativeDisparity + Elevation+ (Elevation|ID)');
...
disp('PM by relative disparity and Elevation model vs PM by disparity comparison')
if model_comp.pValue <0.05
    fprintf('%s: relative disparity and elevation independently contribute to PM, pval=%f df=%d\n',task, model_compAvD.pValue(2),model_compAvD.DF(2));
else
    fprintf('%s: relative disparity and elevation are not independent factors of  PM, pval=%.2f df=%d\n',task, model_compAvD.pValue(2),model_compAvD.DF(2));
end


if saveLME % save stats 
     savelmefile=fullfile('.',ResultsDir, [basename '_' task '_lme_moon_PM_vs_relativedisparity.txt']);
     diary(savelmefile)
...
xvectoru= [minRelativeDisparity:.5: maxRelativeDisparity];
xvectord= [maxRelativeDisparity:-.5: minRelativeDisparity];
...
scatter(disparity_data.RelativeDisparity, disparity_data.Ratio_Visual_Angle, markerScale, subjectcolor2, 'o', 'filled');


yline(1,'Color',[.8 .8 .8 ],'LineWidth',3)
xlabel ('Relative Disparity (degrees)')
ylabel ('Perceptual Magnification') 
ylim([0 maxRatio])
xlim ([minRelativeDisparity maxRelativeDisparity])
